...
Lately I have to output a lot of code, specifically using LLMs and agents for that.
Raw result is done, but now during the maintenance phase, if I find the resulting code and understandability of the code base are not ideal, so to say.
That leads me to try to search for ways to structure code that can act as a "cage" for AI - constraining it to reduce bugs and minimize the time I spend manually correcting its output.
...
The example with regular user and admin user is relatively easy to fix, especially in TypeScript by introducing value types, through to utilize so called [brand types in TypeScript](https://www.learningtypescript.com/articles/branded-types) for user ids.
...
Here is how could the above handler look like using this service approach:
...
By the "cage" I mean set of rules or constraints that from one side do not limit LLM on the outputting the code, but from the another side, guide it automatically to the right approach for this particular project.
...
For instance: duplication detection, which after a specific threshold of duplications fails the tool, and LLM is forced to de-duplicate the code which has been added.

The question is if I can use the compiler, from one standpoint forcing LLM to follow it, and from another standpoint help bio-organisms to read and support code easier.
...
And when we implement such logic, we just need not forget to put a call to `ensureTicketIsPayable()` before any operation that requires the ticket to be in `reserved` state.
